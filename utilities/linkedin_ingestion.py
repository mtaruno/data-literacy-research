...
def popup():
    ...
    try:
        if (
            browser.find_element_by_class_name("msg-overlay-list-bubble--is-minimized")
            is not None
        ):
            pass
        ...
        try:
            if (
                browser.find_element_by_class_name("msg-overlay-bubble-header")
                is not None
            ):
...
job_container = soup.find_all(
    "li",
    {
        "class": "jobs-search-results__list-item occludable-update p0 relative ember-view"
    },
)
    ...
    for post in postings:
        ...
        try:
            ...
            result = page.find_all(
                "li", {"class": "jobs-description-details__list-item t-14"}
            )
